    ...
    public void run(String... args) {
		...
		//	connection.publish("test", "SpringBoot Hello".getBytes(), QoS.AT_LEAST_ONCE, false);
			...
			while(true) {
				...
				File homeDirectory = new File("/home/ilya/parking/snapshot");
				String[] envp = new String[1];
				String[] envp1 = new String[2];
				//String string = "E121MM750";
				fn = string+"_"+convertDateToString()+".jpg";
				envp[0] = "/home/ilya/parking/snapshot/"+fn; //
				process = Runtime.getRuntime().exec(String.format("ffmpeg -y -loglevel fatal -rtsp_transport tcp -i rtsp://admin:ahH2OoVJLy_@178.140.0.246:1554 -frames:v 2 -r 1 -s 640x480 "+envp[0], envp, homeDirectory));
				process.waitFor();
				// System.out.println(envp[0]+": done");
				
				envp1[0] = "frm/"+string;
				envp1[1] = fn;
				process = Runtime.getRuntime().exec("./minio-upload.sh " + envp1[0] + " " + envp1[1],envp1, homeDirectory);
				process.waitFor();
				// System.out.println("=============================================");
